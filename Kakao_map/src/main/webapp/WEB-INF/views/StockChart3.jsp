	...
	<div>
		...
		<article style="margin: 0 0 100px 0;">
			...
			<table>
				...
				<tbody>
					<tr>
						<td id="nowP" colspan="2" rowspan="2">${svo3.nowPrice }<br>전일대비

						<c:set value="${fn:substring(svo3.vsyesterday,0,2)}" var="updown"/>
						<c:if test="${fn:startsWith(updown,'상승') }">
							<span style="color: blue;" >↑${fn:substring(svo3.vsyesterday,2,5) } </span>
						</c:if>
						<c:if test="${fn:startsWith(updown,'하락') }">
							<span style="color: red;">↓${fn:substring(svo3.vsyesterday,2,5) } </span>
						</c:if >						
						/
						<c:set value="${fn:substring(svo3.dungRakrate,0,3) }" var="plus"/>
						<c:if test="${fn:startsWith(plus,'플러스') }">
							<span style="color: blue;"> +${fn:substring(svo3.dungRakrate,3,7) }%</span>
						</c:if>
						<c:set value="${fn:substring(svo3.dungRakrate,0,4) }" var="minus"/>
						<c:if test="${fn:startsWith(minus,'마이너스') }" >
							<span style="color: red;"> -${fn:substring(svo3.dungRakrate,4,8) }% </span>
						</c:if>
						</td>
						
						<td id="data">전일 ${svo3.yesterdayPrice }</td>
						<td id="data">고가 ${svo3.highPrice }</td>
						<td id="data">거래량 ${svo3.georaeryang }</td>
					</tr>
					<tr>
						<td id="data">시가 ${svo3.sigaPrice }</td>
						<td id="data">저가 ${svo3.lowPrice }</td>
						<td id="data">거래대금 ${svo3.tradingValue }</td>
				</tbody>
			</table>
		</article>
	</div>
