...
--JDBC용 SQL
insert into  c_member values( (select nvl(max(c_mem_idx),0)+1 from c_member),?,?,?,?,?,default,default)                           



select * from c_member;

select * from c_member where c_mem_idx=1;

select * from c_member where c_mem_id='이길동';

select * from c_member where c_mem_id='one';

update c_member set c_mem_name=?,c_mem_pwd=?,c_mem_zipcode=?,c_mem_address=?,c_mem_grade=? where c_mem_idx=? 
